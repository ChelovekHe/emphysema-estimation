...
::GetOutput() {
  ...
  if ( ! m_Read ) {
    ...
    // column-major order.
    for ( long int z = 0; z < header.size.at(2); ++z ) {	  
      for ( long int x = 0; x < header.size.at(0); ++x ) {
	for ( long int y = 0; y < header.size.at(1); ++y ) {
	  if ( !is.good() ) {
  ...
  return m_Image;
    ...
    // Octave uses column-major order
    ...
    // 	idx[Dimension-i] = r % size[Dimension-i];
    // 	r = (r - idx[Dimension-i]) / size[Dimension-i];	
    //   }
    //   PixelType p;
    //   is >> p;
    ...
    //   for ( unsigned int dim = 1; dim <= header.dimension; ++dim ) {
  //     // Read 
  //   for ( size_t z = 0; z < 
  ...
  //     if ( !is.good() ) {
  // 	throw std::invalid_argument( "Not enough values in file" );
  // 	// We need to read a value and then cast it to the requested pixel type
  //     }
  //     PixelType p;
  //     is >> p;
  //     iter.Set( p );
