/* 
   Train a CMS model with continuous cluster labeling
*/
...
int main(int argc, char *argv[]) {
  ...
  TCLAP::ValueArg<std::string> 
    bagLabelsArg("p", 
		 "bag-labels", 
		 "Path to bag label vector stored as csv file. Should have one label for each bag.",
		 true,
		 "",
		 "path", 
		 cmd);
  ...
  // Parse the bag membership into an array
  ...
  // Parse the bag labels into a vector
  ...
  readTextSequence<ElementType>(bagLabelsIs,
				std::back_inserter(bagLabelsBuffer),
				rowSep);
  VectorType bagLabels(bagLabelsBuffer.size());
  std::copy( bagLabelsBuffer.cbegin(),
	     bagLabelsBuffer.cend(),
	     bagLabels.data() );
  ...
  // Now we should have the range of numbers [0, bagLabels.size())
  if ( bagMembershipCopy.size() != bagLabels.size() ) {
    std::cerr << "Size of bag membership does not match number of labels"
	      << std::endl
	      << "Unique bag: " << bagMembershipCopy.size() << std::endl
  ...
  // the frint and back.
  if ( bagMembershipCopy.front() != 0 || bagMembershipCopy.back() != bagLabels.size() - 1 ) {
    std::cerr << "Bag labels must belong to the range [0," << bagLabels.size() - 1 << ")" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::string lossesOutputPath = outputPath + "_training_loss.txt";
